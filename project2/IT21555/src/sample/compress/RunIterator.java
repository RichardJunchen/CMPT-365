package sample.compress;/* RunIterator.java */
...
public class RunIterator implements Iterator {
  ...
  public int[] next() {
    ...
    // Replace the following line with your solution.
    int[] copyRun = new int[4];
    copyRun[0]=cur.item[3];
    copyRun[1]=cur.item[0];
    copyRun[2]=cur.item[1];
    copyRun[3]=cur.item[2];
    if (hasNext()) {
      cur=cur.next;
    }else{
